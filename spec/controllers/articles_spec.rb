...
describe 'News API', type: :request do
  ...
  context 'when parameters are valid' do
    before do
      post '/articles', params: article_params.to_json, headers: { 'CONTENT_TYPE' => 'application/json' }
    end
    ...
    it 'adds an article to the database' do
      ...
      actual = JSON.parse(response.body).map { |article| article.except('created_at', 'updated_at') }
    ...
    it 'returns JSON of a created article' do
      ...
      actual = JSON.parse(response.body).except('created_at', 'updated_at')
      expect(actual).to eq(expected)
    end
  end
  ...
  context 'when title is missing' do
    let(:invalid_params) do
      { article: article_params[:article].merge(title: nil) }
    end

    before do
      post '/articles', params: invalid_params.to_json, headers: { 'CONTENT_TYPE' => 'application/json' }
    end
  ...
  context 'when title is longer than 40 characters' do
    let(:invalid_params) do
      { article: article_params[:article].merge(title: 'New exhibition at the Royal Botanic Garden Edinburgh') }
    end
  ...
  context 'when content is missing' do
    let(:invalid_params) do
      { article: article_params[:article].merge(content: nil) }
    end
  ...
  context 'when author is missing' do
    let(:invalid_params) do
      { article: article_params[:article].merge(author: nil) }
    end
  ...
  context 'when published_at is missing' do
    let(:invalid_params) do
      { article: article_params[:article].merge(published_at: nil) }
    end
  ...
  context 'when article by given ID exists' do
    ...
    let!(:article) do
      post '/articles', params: article_params, headers: { 'CONTENT_TYPE' => 'application/json' }
    ...
    it 'returns JSON of a corresponding article' do
      ...
      response_body.except!('created_at', 'updated_at')

      expect(response_body).to eq(expected.except('created_at', 'updated_at'))
    end
  end
    ...
    context 'when there are articles in the system' do
      ...
      before do
        article_params.each do |params|
          post '/articles', params: params.to_json, headers: { 'CONTENT_TYPE' => 'application/json' }
          fail('Cannot create an article') unless response.status == 201
        end
      end
      ...
      it 'returns articles collection ordered by published_at descending' do
        ...
        expected = article_params.map.with_index(1) do |params, index|
          params[:article]['id'] = index
          params[:article].stringify_keys
        end.sort do |a, b|
          Date.parse(b['published_at']) <=> Date.parse(a['published_at'])
        end

        actual = JSON.parse(response.body).map do |article|
          article.except('created_at', 'updated_at')
        end
